...
function Section({ title, content }) {
  return (
    <section className="mt-3 print:break-inside-avoid">
      <h4 className="text-sm font-semibold text-slate-800 print:text-black">{title}</h4>
      <p className="mt-1 text-sm text-slate-700 print:text-black whitespace-pre-line">
        {content}
      </p>
    </section>
  );
  ...
  return (
    <article
      className="rounded-xl border border-slate-100 bg-slate-50/80 p-4 
                 print:break-inside-avoid print:bg-white"
    >
      <header className="flex items-baseline justify-between gap-4 mb-2">
        <div>
          <h3 className="text-xl font-semibold text-slate-900 print:text-black">
            {cat.name}
          </h3>
          {cat.tagline && (
            <p className="text-sm text-sky-700 mt-1 print:text-black">
              {cat.tagline}
            </p>
          )}
        </div>

        <span className="text-xs font-medium uppercase tracking-wide text-slate-500 print:text-black">
          Match score: {score}
        </span>
      </header>
      ...
      {cat.jobLinks && (
        <div className="mt-3">
          <h4 className="text-sm font-semibold text-slate-800 print:text-black">
            Explore real job postings
          </h4>
          <ul className="mt-1 text-sm text-sky-700 space-y-1 print:text-black">
